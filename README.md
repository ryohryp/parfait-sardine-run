...
- コメントごとに「いいね」ボタンを表示し、`POST /wp-json/psr/v1/like` でトグル操作します。クライアント識別子は `localStorage` に保存した ID を `X-PSR-Client` ヘッダーで送信し、レスポンスの `like_count` と `liked` を即座に UI に反映します。コメント一覧取得時も同ヘッダーを付けることで既に「いいね」済みか判別できます。【F:index.html†L140-L209】【F:index.html†L712-L965】【F:index.html†L900-L1013】
